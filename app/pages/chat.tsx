...
// ===== Config =====
const CHAT_ENDPOINT = `${process.env.API_BASE_URL}/user-chat/chat`;
...
export default function ChatScreen() {
  ...
  const send = async () => {
    ...
    // 先に自分の発言を表示
    const me: Msg = { id: `${Date.now()}`, text: content, from: 'me', createdAt: Date.now() };
    ...
    try {
      ...
      const replyText =
        typeof data === 'object' && data !== null
          ? (data.generated_json?.answer && typeof data.generated_json.answer === 'string'
              ? data.generated_json.answer
              : (typeof data.generated_text === 'string' && data.generated_text) || '(応答なし)')
          : (typeof data === 'string' ? data : '(応答なし)');

      // ★ レスポンスの form をそのまま次回のリクエストに載せる
      if (typeof data === 'object' && data !== null && data.form && typeof data.form === 'object') {
        setForm({
      ...
      setMessages(prev => [
        { id: `b-${Date.now()}`, text: replyText, from: 'bot', createdAt: Date.now() },
        ...prev,
      ]);
    } catch (e: any) {
      setMessages(prev => [
